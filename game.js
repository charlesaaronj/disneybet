...
const PARK_THEMES = {
  "Magic Kingdom": {
    hero:   "radial-gradient(circle at top, #6d28d9 0%, #1f143b 45%, #05030a 100%)",
    nav:    "rgba(18, 12, 48, 0.96)",
    avatar: "linear-gradient(135deg, #f97316, #facc15)"
  },
  "EPCOT": {
    hero:   "radial-gradient(circle at top, #0f766e 0%, #0b3b6b 40%, #020617 100%)",
    nav:    "rgba(7, 21, 48, 0.96)",
    avatar: "linear-gradient(135deg, #38bdf8, #a855f7)"
  },
  "Hollywood Studios": {
    hero:   "radial-gradient(circle at top, #f97316 0%, #7c2d12 35%, #020617 100%)",
    nav:    "rgba(15, 10, 32, 0.96)",
    avatar: "linear-gradient(135deg, #fbbf24, #f97316)"
  },
  "Animal Kingdom": {
    hero:   "radial-gradient(circle at top, #16a34a 0%, #065f46 40%, #020617 100%)",
    nav:    "rgba(4, 30, 20, 0.96)",
    avatar: "linear-gradient(135deg, #4ade80, #22c55e)"
  },
  "Dollywood": {
    hero:   "radial-gradient(circle at top, #f97316 0%, #9a3412 40%, #020617 100%)",
    nav:    "rgba(15, 9, 24, 0.96)",
    avatar: "linear-gradient(135deg, #f97316, #fde68a)"
  }
};
...
function runRevealAnimation(){
  ...
  setTimeout(() => {
    ...
    // Confetti for correct guesses on real answers
    ...
    if(authorLineSummary){
      if(r.wrongGuessCount > 0 && !isGhostAnswer){
        ...
        authorLineSummary.textContent =
          `⭐ Author bonus: ${author ? author.name : "The author"} earned ` +
          `${b} point${b !== 1 ? "s" : ""} from ${w} wrong guess${w !== 1 ? "es" : ""}.`;
      } else {
        authorLineSummary.textContent = "";
      }
    }
    ...
    if(houseLineText){
      if(r.houseBonusApplied){
        ...
        houseLineText.textContent =
          `🎟️ House bonus: ${r.houseBonusResolved} split evenly between ${names}.`;
      } else if(r.houseBonusAmount > 0){
        houseLineText.textContent =
          `🎟️ House bonus: ${r.houseBonusReason || "Not applied."}`;
      } else {
        houseLineText.textContent = "";
      }
    }
        ...
        row.innerHTML = `
          <div>
            ...
            <div class="wsd-score-meta">
              Guess: ${guessName || "—"} · Wager: ${wager ? wager.amount : 0}
            </div>
          </div>
...
function wireEvents(){
  ...
  $("wsd-nav-round")?.addEventListener("click",()=>{
    if(!gameState)return;
    const roundScreen=ROUND_SCREENS.includes(gameState.screen)?gameState.screen:"setup-question";
    showScreen(roundScreen);
  });
  $("wsd-nav-scores")?.addEventListener("click",()=>{
    if(!gameState)return;
    if(gameState.finalBonusesApplied){renderFinalResults();showScreen("game-end");}
